...
[![Go Report Card](https://goreportcard.com/badge/github.com/ccnlab/leabrax)](https://goreportcard.com/report/github.com/ccnlab/leabrax)
[![GoDoc](https://godoc.org/github.com/ccnlab/leabrax?status.svg)](https://godoc.org/github.com/ccnlab/leabrax)
...
* 3/2019: Python interface is up and running!  See the `python` directory in `leabra` for the [README](https://github.com/ccnlab/leabrax/blob/master/python/README.md) status and how to give it a try.  You can run the full `examples/ra25` code using Python, including the GUI etc.
...
* `ActParams` (in [act.go](https://github.com/ccnlab/leabrax/blob/master/leabra/act.go)), `InhibParams` (in [inhib.go](https://github.com/ccnlab/leabrax/blob/master/leabra/inhib.go)), and `LearnNeurParams` / `LearnSynParams` (in [learn.go](https://github.com/ccnlab/leabrax/blob/master/leabra/learn.go)) provide the core parameters and functions used, including the X-over-X-plus-1 activation function, FFFB inhibition, and the XCal BCM-like learning rule, etc.  This function-based organization should be clearer than the purely structural organization used in C++ emergent.
...
* There is also a `leabra.LeabraLayer` and `leabra.LeabraPrjn` interface, defined in [leabra.go](https://github.com/ccnlab/leabrax/blob/master/leabra/leabra.go), which provides a virtual interface for the Leabra-specific algorithm functions at the basic level.  These interfaces are used in the base leabra code, so that any more specialized version that embeds the basic leabra types will be called instead.  See `deep` sub-package for implemented example that does DeepLeabra on top of the basic `leabra` foundation.
...
* [deep](https://github.com/ccnlab/leabrax/blob/master/deep) has the DeepLeabra mechanisms for simulating the deep neocortical <-> thalamus pathways (wherein basic Leabra represents purely superficial-layer processing)
* [pbwm](https://github.com/ccnlab/leabrax/blob/master/rl) has basic reinforcement learning models such as Rescorla-Wagner and TD (temporal differences).
* [pbwm](https://github.com/ccnlab/leabrax/blob/master/pbwm1) has the prefrontal-cortex basal ganglia working memory model (PBWM).
* [hip](https://github.com/ccnlab/leabrax/blob/master/hip) has the hippocampus specific learning mechanisms.
